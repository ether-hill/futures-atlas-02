...
(function () {
  ...
  var FA_PROJECTS = [
    ...
    // listed here the only way to either of them on a phone was a 63x15 link in
    // the bundle's own corner. The sheet lists a project's pages at full size.
    { name: "The Odds", path: "/theodds", theme: "dark", draft: true, pages: [
      { name: "Home", path: "/theodds" },
      { name: "Research", path: "/theodds/research" },
      { name: "Stats", path: "/theodds/stats" },
    ] },
  ...
  /*
    DECLARED HERE, not further down with its comment block, and that matters.

    `var` hoists the NAME but not the assignment, so while this line sat above
    the declaration, IS_PRODUCTION was `undefined` when IS_EDITOR read it, and
    `!undefined` is true. Every visitor carrying a stale `fa_editor=1` cookie
    was therefore treated as an editor ON PRODUCTION, and FA_LISTED handed the
    switcher and the mobile sheet every DRAFT project. Their URLs are gated and
    404, so nothing unreleased could be read, but the names were on show. The
    comment below this once claimed the opposite.
  */
  ...
  */
  /* IS_PRODUCTION is declared ABOVE IS_EDITOR; see the note there. */
  ...
  function mount() {
    ...
    // A host page that server-rendered the generated footer already has the
    // right one, and it belongs to React — replacing it below tore a live node
    // out of the tree and tripped a hydration mismatch. Only a footer with no
    // data-fa-footer marker is one of the stale hardcoded copies.
    var hostFooter = document.querySelector("footer.fa-foot[data-fa-footer]");
    if (isProject && !root.hasAttribute("data-fa-no-footer") && !hostFooter) {
