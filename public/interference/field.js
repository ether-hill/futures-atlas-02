...
window.FIELD = (function () {
...

  {
    ...
    frag: [
      ...
      "     behind it. Adding the two stands a lattice of still points in front",
      "     of the wall.",
      "",
      "     R is well under one because a wall with two holes cut in it is not a",
      "     mirror. At full strength the two families are equal everywhere and",
      "     the whole near side reads as a diamond lattice, with the circles",
      "     only visible right at the lamp. Weak, the lattice sits where it",
      "     belongs \u2014 the band near the wall, where what has come back is still",
      "     comparable to what is going out \u2014 and everywhere else the field",
      "     reads as what it is: circles. */",
      "  const float R = 0.34;",
      "  vec2 near = circ(p, lamp) + R*circ(p, vec2(2.0*XB - SX, 0.0));",
...

  {
    ...
    frag: [
      ...
      "",
      "  const float R = 0.34;   /* see the note on the panel before this one */",
      "  vec2 near = circ(p, lamp) + R*circ(p, vec2(2.0*XB - SX, 0.0));",
