...
export function profileFor(matched: number, total: number, over: number, under: number): Profile {
  ...
  return { name: "The Sceptic", desc: "You resist the story hard, past the point where the thing actually happened. Doubt on its own is easy; calibration means getting it right in both directions.", lblNote: "but doubt overshot the facts" };
}
